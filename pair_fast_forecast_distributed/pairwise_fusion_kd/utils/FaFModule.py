...
class FaFModule(object):
	...
	def step(self,data,batch_size, center_agent, forecast_num = 1, rank = 0, config = Config):
		...
		if self.MGDA:
			...
			result = self.model(bev_seq, points_teacher, trans_matrices, num_agent, to_new_trans_mat_list, supervise_data, batch_size=batch_size, center_agent = center_agent, delta_t = delta_t, forecast_num = forecast_num,rank = rank, config = config)
		...
		if config.forecast_loss == 'True':
			...
			# loss += fore_loss
			loss = 0.00000000001 * loss + fore_loss
		if config.forecast_KD == 'True':
			loss = loss + 0.2 * result['KDLoss']
		...
		if self.config.pred_type in ['motion','center'] and not self.only_det:
			...
			if ((config.forecast_loss == 'True') and (config.forecast_KD == 'True')):
				return loss.item(),loss_cls.item(),loss_loc.item(), fore_loss, result['KDLoss']
			elif ((config.forecast_loss == 'True') and (config.forecast_KD == 'False')):
				return loss.item(),loss_cls.item(),loss_loc.item(), fore_loss
			else:
