...
def apply_box_global_transform(trans_matrices_map, batch_box_preds, batch_cls_preds, anchors, code_type, config,
                               batch_motion=None):
    """
    ...
    for box_preds, cls_preds, anchors in zip(
        ...
        global_points = np.dot(trans_matrices_map, np.vstack(
            (local_points, np.zeros(local_points.shape[1]), np.ones(local_points.shape[1]))))[:2, :]
...
def apply_box_global_transform_af_localnms(trans_matrices_map, class_selected, box_scores_pred_cls):
    ...
    trans_matrices_map = torch.squeeze(trans_matrices_map)
    trans_matrices_map = trans_matrices_map.cpu().detach().numpy()
    global_points = np.dot(trans_matrices_map,
                           np.vstack((local_points, np.zeros(local_points.shape[1]), np.ones(local_points.shape[1]))))[
                    :2, :]
    # print(global_points.shape)
    global_points[0, :] = - global_points[0, :]
    global_points = global_points.T
...
def apply_nms_global_scene(all_points_scene, cls_preds_scene):
    ...
    for i in range(total_scores.shape[1]):
        ...
        class_selected.append({'pred': all_points_scene[selected_idx], 'score': total_scores[selected_idx, i],
...
def apply_box_local_transform(class_selected_global, trans_matrices_map):
    ...
    local_points = np.dot(trans_matrices_g2l, np.vstack(
        (global_points, np.zeros(global_points.shape[1]), np.ones(global_points.shape[1]))))[:2, :]
...
def apply_nms_det(batch_box_preds, batch_cls_preds, anchors, code_type, config, batch_motion=None):
    ...
    for box_preds, cls_preds, anchors in zip(
        ...
        for i in range(total_scores.shape[1]):
            ...
            class_selected.append({'pred': box_corners[selected_idx], 'score': total_scores[selected_idx, i],
                                   'selected_idx': selected_idx})
...
def cal_local_mAP(config, data, det_results, annotations):
    ...
    for p in range(pred_len):
        ...
        for k in range(gt_corners.shape[0]):
            gt_box = np.array([gt_corners[k, 0, 0], gt_corners[k, 0, 1], gt_corners[k, 1, 0], gt_corners[k, 1, 1],
                               gt_corners[k, 2, 0], gt_corners[k, 2, 1], gt_corners[k, 3, 0], gt_corners[k, 3, 1]])
            if k == 0:
                gt_boxes_frame = np.array(
                    [gt_corners[k, 0, 0], gt_corners[k, 0, 1], gt_corners[k, 1, 0], gt_corners[k, 1, 1],
                     gt_corners[k, 2, 0], gt_corners[k, 2, 1], gt_corners[k, 3, 0], gt_corners[k, 3, 1]])
            else:
        ...
        for k in range(pred_corners.shape[0]):
            ...
            detection_result = np.array(
                [pred_corners[k, 0, 0], pred_corners[k, 0, 1], pred_corners[k, 1, 0], pred_corners[k, 1, 1],
                 pred_corners[k, 2, 0], pred_corners[k, 2, 1], pred_corners[k, 3, 0], pred_corners[k, 3, 1],
                 cls_score[k]])
            if k == 0:
