...
from utils.model import STPN, STPN_KD, MapExtractor, MotionNet, MotionRNN, forecast_lstm, Motion_Prediction_LSTM, lidar_encoder, lidar_decoder, lidar_decoder_kd, conv2DBatchNormRelu, Sparsemax, adafusionlayer,sigmoidfusionlayer, pairfusionlayer,pairfusionlayer_1, pairfusionlayer_2, pairfusionlayer_3 ,pairfusionlayer_4
import numpy as np
import copy
...
class FaFMIMONet_256_32_32(nn.Module):
    def __init__(self, config, in_channels=13, shared_img_encoder='unified', forecast_num = 3):
        ...
        self.KDLoss = nn.KLDivLoss(size_average=True, reduce=True, reduction = 'sum')
        self.box_code_size = config.box_code_size
        self.category_num = config.category_num
        self.use_map = config.use_map
        self.anchor_num_per_loc = len(config.anchor_size)
    ...
    def forward(self, bevs, points_teacher, trans_matrices, num_agent_tensor, to_new_trans_mat_list,  supervise_data, vis=None, training=True, MO_flag=True, inference='activated', batch_size=2, center_agent = 0, delta_t = [0,10,10,10,10], rank = 0, forecast_num = 1, mode = 'train', config = Config, scene = '80_0'):
        # self.u_encoder.eval()
        # self.adafusion.train()
        device = bevs.device
        ...
        scene = [scene, delta_t, self.forecast_flag]
        for i in range(batch_size):
            temp = self.adafusion(feat_list[i][0:num_agent_tensor[i][0]], scene)
        ...
        # if not vis is None:
        #     x = torch.cat([x,vis],axis=1)
        kl_loss_mean = nn.KLDivLoss(size_average=True, reduce=True, reduction='batchmean')
        kd_weight = 1
        if config.forecast_KD == 'True':
        ...
        if config.forecast_loss == 'True' and config.forecast_KD == 'True':
            KD_loss = self.Forecast_loss(local_com_mat_update, x_3_teacher)
            result = {'loc': loc_preds,
                    'cls': cls_preds,
