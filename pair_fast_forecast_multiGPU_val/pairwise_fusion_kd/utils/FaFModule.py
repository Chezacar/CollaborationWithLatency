...
class FaFModule(object):
	...
	def loss_calculator(self,result,anchors,reg_loss_mask,reg_targets,labels,N,motion_labels = None,motion_mask=None):
		...
		if self.code_type in ['corner_1','corner_2','corner_3']:
				target = reg_targets[reg_loss_mask].reshape(-1,1,2)
				flip_target = torch.stack([target[:,0],target[:,3],target[:,4],target[:,1],target[:,2]],dim=-2)
				pred = result['loc'][reg_loss_mask].reshape(-1,1,2)
	...
	def vector_2_mat(self, batch_size, ref_cs_rec, ref_pose_rec, inverse_flag = True, standard_matrix = 0):
		...
		for batch in range(batch_size):
			...
			if inverse_flag == True:
				...
				# standard_global_from_car = transform_matrix(np.sum([ref_pose_rec_translation],axis=0), Quaternion(ref_pose_rec_rotation), inverse = inverse_flag)
				standard_global_from_car = transform_matrix(ref_pose_rec_translation, Quaternion(ref_pose_rec_rotation), inverse = inverse_flag)
			...
			if inverse_flag == False:
				...
				# temp_matrix[0][-1] -= 0.94618253 
				# temp_matrix[1][-1] += 0.93997967
	...
	def step(self,data,batch_size, center_agent, forecast_num, config):
		...
		for iter in range(batch_size):
			...
			delta_t.append(torch.unsqueeze(torch.tensor(delta_t_temp),0))
		# with torch.autograd.set_detect_anomaly(True):
		delta_t = torch.cat(delta_t,0)
		delta_t = delta_t.to('cuda')
		...
		if self.MGDA:
			...
			result = self.model(bev_seq, points_teacher, trans_matrices, num_agent, to_new_trans_mat_list, supervise_data, batch_size=batch_size, center_agent = center_agent, delta_t = delta_t, forecast_num = forecast_num, config=config, scene = filename)
		...
		if config.forecast_loss == 'True':
			loss = result['forecast_loss'] + 0.00000000000001 * loss
			fore_loss = result['forecast_loss']
			kd_loss = result['KDLoss']
			loss = kd_loss + loss
		if self.MGDA:
		...
		if self.config.pred_type in ['motion','center'] and not self.only_det:
			...
			if config.forecast_loss == 'True' and config.forecast_KD == 'True':
				return loss.item(),loss_cls.item(),loss_loc.item(),result['forecast_loss'], kd_loss
	...
	def predict_all(self,data,batch_size,validation=True, center_agent = 0, forecast_num = 1, config = Config):
		...
		if self.MGDA:
			...
			with torch.no_grad():
				result= self.model(bev_seq, bev_seq, trans_matrices, num_agent_tensor, to_new_trans_mat_list, supervise_data = supervise_data, batch_size=batch_size, center_agent = center_agent, delta_t = delta_t, forecast_num = forecast_num, mode = 'val', config = config, scene = filename)
...
class FaFModuleKD(object):
	...
	def step(self, data, batch_size):
		...
		if layer==4:
			...
			student_x3 = x_3.permute(0, 2, 3, 1).reshape(1 *batch_size*32*32, -1)
