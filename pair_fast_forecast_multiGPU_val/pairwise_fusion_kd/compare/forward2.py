...
class FaFModule(object):
	...
	def step(self,data,batch_size, center_agent, forecast_num = 1, rank = 0, config = Config):
		...
		if self.MGDA:
			...
			result = self.model(bev_seq, trans_matrices, num_agent, to_new_trans_mat_list, supervise_data, batch_size=batch_size, center_agent = center_agent, delta_t = delta_t, forecast_num = forecast_num,rank = rank, config = config)
		...
		if config.forecast_loss == 'True':
			...
			# loss += fore_loss
			loss = loss * 0.0000000000000001 + fore_loss
