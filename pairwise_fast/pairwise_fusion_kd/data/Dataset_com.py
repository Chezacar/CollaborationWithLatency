...
class CarscenesDataset(Dataset):
    def __init__(self, padded_voxel_points_example, label_one_hot_example, reg_target_example, reg_loss_mask_example,
                 ...
                 config_global=None, agent_list = ['agent0', 'agent1', 'agent2', 'agent3', 'agent4'], split=None, cache_size=10000, val=False, latency_lambda = [10,10,10,10,10]):
        """
        This dataloader loads single sequence for a keyframe, and is not designed for computing the
         spatio-temporal consistency losses. It supports train, val and test splits.
    ...
    def __getitem__(self, idx):
        ...
        for agent in range(len(self.latency_lambda)):
            data_return[agent] = {}
            seq_file = self.seq_dict[center_agent][agent][idx]
            # seq_file = self.seq_files[idx]
            empty_flag = False
            ...
            if gt_data_handle == 0:
                ...
                if self.val:
                    ...
                    data_return[agent]['num_sensor'] = 0
                    data_return[agent]['trans_matrices'] = np.zeros((5, 4, 4))
                    # data_return['padded_voxel_points'].append(padded_voxel_points)
                    # data_return['label_one_hot'].append(label_one_hot)
                    # data_return['reg_target'].append(reg_target)
                    # data_return['reg_loss_mask'].append(reg_loss_mask)
                    # data_return['anchors_map'].append(anchors_map)
                    # data_return['vis_maps'].append(vis_maps)
                    # data_return['gt_max_iou'].append([{"gt_box": [[0, 0, 0, 0], [0, 0, 0, 0]]}])
                    # data_return['filename'].append([seq_file])
                    # data_return['target_agent_id'].append(0)
                    # data_return['num_sensor'].append(0)
                    # data_return['trans_matrices'].append(np.zeros((5, 4, 4)))
                    continue
            else:
                gt_dict = gt_data_handle.item()
                if len(self.cache) < self.cache_size:
            ...
            if empty_flag == False:
                ...
                if self.val:
                    ...
                    # data_return['anchors_map'].append(anchors_map)
                    # data_return['vis_maps'].append(vis_maps)
                    # data_return['gt_max_iou'].append([ {"gt_box": gt_max_iou}])
                    # data_return['filename'].append([seq_file])
                    # data_return['target_agent_id'].append(target_agent_id)
                    # data_return['num_sensor'].append(num_sensor)
                    # data_return['trans_matrices'].append(trans_matrices)
                    data_return[agent]['padded_voxel_points'] = padded_voxel_points
                    ...
                    continue
        # data_return_list = []
        # for key1 in data_return:
        #     if key1 != 'center_agent':
        #         temp_list = []
        #         for key2 in data_return[key1]:
        #             temp_list.append(data_return[key1][key2])
        #         data_return_list.append(temp_list)
        #     else:
        #         data_return_list.append(data_return['center_agent'])
        # if not self.val:
            # return data_return['padded_voxel_points'], data_return['label_one_hot'], data_return['reg_target'], data_return['reg_loss_mask'], data_return['anchors_map'].append(anchors_map), data_return['vis_maps'], data_return['gt_max_iou'],data_return['filename'],data_return['target_agent_id'],data_return['num_sensor'],data_return['trans_matrices'], center_agent
        return data_return
